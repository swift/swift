...
MIXRegistry::MIXRegistry(const JID& ownJID, IQRouter* iqRouter, XMPPRoster* xmppRoster, StanzaChannel* stanzaChannel, PresenceSender* presenceSender) : ownJID_(ownJID), iqRouter_(iqRouter), xmppRoster_(xmppRoster), stanzaChannel_(stanzaChannel), presenceSender_(presenceSender) {
    initialRosterPopulationConnection_ = xmppRoster_->onInitialRosterPopulated.connect([=] {
        ...
        for (auto item : xmppRoster_->getItems()) {
            if (item.isMIXChannel()) {
                ...
                if (i == entries_.end()) {
                entries_.insert(std::make_pair(item.getJID(), std::make_shared<MIXImpl>(ownJID_, item.getJID(), iqRouter_, stanzaChannel_, presenceSender_)));
                }
            }
        }
        onSyncSuccess();
...
void MIXRegistry::handleJIDAdded(const JID& jid) {
    if (xmppRoster_->isMIXChannel(jid)) {
        ...
        if (i == entries_.end()) {
            entries_.insert(std::make_pair(jid, std::make_shared<MIXImpl>(ownJID_, jid, iqRouter_, stanzaChannel_, presenceSender_)));
        }
        onChannelJoined(jid);
    }
}
